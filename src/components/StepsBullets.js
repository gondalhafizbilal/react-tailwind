...
const stepsData = [
  { name: 'Property detail', status: 'complete' },
  { name: 'Rent roll', status: 'upcoming' },
  { name: 'Financing', status: 'upcomping' },
  { name: 'Congratulations', status: 'upcoming' },
]
...
function StepsBullets({ step }) {
  const [steps, setSteps] = useState([]);
  useEffect(() => {

    setSteps(stepsData.map((sdata, index) => {
      if (index === step - 1) {
        return { ...sdata, "status": "current" }
      } else if (index < step - 1) {
        return { ...sdata, "status": "complete" }
      } else {
        return { ...sdata, "status": "upcoming" }
      }
    }))
  }, [step])
              ...
              {step.status === 'complete' ? (
                <>
                  {stepIdx !== steps.length - 1 ? (
                    <div className="absolute left-2.5 top-3 -ml-px mt-0.5 h-full w-0.5 bg-black" aria-hidden="true" />
                  ) : null}
                  <a className="group">
